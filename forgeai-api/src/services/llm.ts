...
export interface LlmService {
  ...
  generateStructuredAnswer(prompt: string, systemPrompt?: string): Promise<string>
}
...
export class OpenAILlmService implements LlmService {
  ...
  async generateStructuredAnswer(prompt: string, systemPrompt?: string): Promise<string> {
    if (!this.apiKey) {
      throw new Error('OpenAI API Key is missing')
    }
...
To enable live LLM completions, define \`GROQ_API_KEY\` in your \`forgeai-api/.env\` file.`
  ...
  async generateStructuredAnswer(_prompt: string, _systemPrompt?: string): Promise<string> {
    return JSON.stringify({
      filePath: 'example.ts',
      startLine: 1,
      endLine: 3,
      originalCode: '// original code placeholder',
      proposedCode: '// proposed fix placeholder',
      explanation: 'Mock mode — no LLM API key configured. Define GROQ_API_KEY in your .env file.',
    })
  }
}
