...
| Phase 5 | Swappable embedding providers (Gemini/Jina fallback, rate-limit retry) | ✅ Complete |
| Phase 6 | GitBrain Lite — scheduled discovery engine, semantic search, curated catalog | ✅ Complete |

## Phase 6: GitBrain Lite — Discovery Engine

### Health Score Formula

The discovery engine computes a `healthScore ∈ [0, 1]` for each repo using only data already available from the GitHub API. It is **not ML-based** and not a black box:

```
raw = log10(stars + 1) * 10         // star term: ~40 pts at 10k stars
    + max(0, 40 - daysSincePush/4)   // freshness: 40 pts today, -1pt/4 days
    + max(0, 20 - (openIssues/max(stars,1)) * 100)  // quality: 20 pts if low issue ratio

healthScore = clamp(raw, 0, 100) / 100
```

Sample scores (stars term only, for illustration):
| Stars | Star term | Notes |
|---|---|---|
| 200 | ~23.0 | minimum threshold in query list |
| 500 | ~27.0 | higher query threshold |
| 5,000 | ~37.0 | popular mid-tier repo |
| 10,000 | ~40.0 | maximum star contribution |

### Manually Triggering a Discovery Run

The weekly discovery job runs automatically every Sunday at 02:00 UTC. To trigger it manually:

1. **Set `ADMIN_USER_ID`** in `forgeai-api/.env` to your own user ID (the `sub` field from your JWT session, which equals your Prisma `User.id`).

2. **Sign in to ForgeAI** (you need a valid session cookie).

3. **POST to the trigger endpoint:**
   ```bash
   curl -X POST https://your-api-url/api/discover/trigger \
     -H "Content-Type: application/json" \
     --cookie "forgeai_session=<your-cookie>"
   ```
   Or call it from the browser console while signed in:
   ```js
   fetch('/api/discover/trigger', { method: 'POST', credentials: 'include' })
     .then(r => r.json()).then(console.log)
   ```

4. **Watch server logs** for `[discovery]` prefixed lines showing per-repo progress.

Rate limit: **once per hour** per user.

### Tags Disclaimer

Tags on discovered repos (`domainTags`, `techTags`, `architectureTags`) are generated by an LLM reading the repo's README and dependency manifest. They are **best-effort classification, not a trained classifier**. They may be wrong, incomplete, or outdated. The UI labels this clearly.

### Embedding Consistency

The discovery corpus maintains a single embedding provider per full refresh cycle. At the start of each weekly run, all existing rows are re-embedded using whichever provider is available (Gemini primary, Jina fallback). The semantic search endpoint detects the dominant provider in the catalog and embeds the user's query with the same model, ensuring vector space consistency.
...
- **Discovery catalog size**: Capped at 300 rows (`DISCOVERY_CATALOG_CAP`). This is a curated sample, not a comprehensive index of GitHub.
